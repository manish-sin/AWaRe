...
def get_date_specific_log(date):
    ...
    df = pd.read_csv("C:/Users/manis/Desktop/awl.csv", index_col='Unnamed: 0')  # C:/Users/manis/Desktop/
    #print(df)
    ...
    for item in app_chart1["Suration"]:
        if (item < thrashold):  # Step 2: operation to be done for each row which are leaa than threshold
            if (item > low_threshold):
                ...
                misc_app = app_chart1["Application"][
                    index]  # getting the name of sub-application to store in "misc_apps"
